...
def network_mask(topo_path, lsmask_path):
    ...
    data = data.where(
        ~((lat > 67) & (lat < 80) & (lon > -180 + 360) & (lon < -100 + 360))
    )
    ...
    data = data.where(
        ~((lat > 70) & (lat < 80) & (lon > -130 + 360) & (lon < -80 + 360))
    )
...
def calc_anom(array_1d, N_time, N_batch, array_mask0=None):
    ...
    for i in range(-(-N_time // N_batch)):
        ...
        array_2d[(i * N_batch) : ((i + 1) * N_batch), :] = (
            array_2d[(i * N_batch) : ((i + 1) * N_batch), :] - avg_val
        )
...
def calc_interannual_anom(df):
    ...
    DS = (
        df.to_xarray()
    )  # get from multi-index back to an xarray for calculating mean seasonal cycle:
...
def import_member_data(
    ensemble_dir_head, ens, member, xco2_path, socat_path, dates, files_ext="zarr"
):
    """
    ...
    chl_clim_path = fs.glob(f"{ensemble_dir_head}/{ens}/{member}/chlclim*.{files_ext}")[
        0
    ]
    ...
    socat_mask_data = xr.open_zarr(socat_path).sel(
        time=slice(str(dates[0]), str(dates[-1]))
    )
    ...
    inputs["pCO2_DIC"] = (
        member_data.pco2_residual
    )  # non temperature component of pCO2 (what we will reconstruct)
    inputs["pCO2"] = (
        member_data.spco2
    )  # Reconstruct pCO2-pCO2T (difference) # actual pco2
    ...
    chl2 = xr.Dataset(
        {"chl_sat": (["time", "ylat", "xlon"], chl_sat.data)},
        coords={
            "time": (["time"], tmp2.time.data),
...
def create_features(df, N_time, N_batch=12):
    ...
    df["mld_anom"] = calc_anom(
        df["mld_log"].values, N_time, N_batch, array_mask0=df["mld"].values
    )
    df["mld_log_anom"] = calc_interannual_anom(df["mld_log"])

    df_mld = df.loc[(df["mld"] > 0), "mld"]
    mld_grouped = df_mld.groupby(
        by=[df_mld.index.get_level_values("time").month, "xlon", "ylat"]
    ).mean()
    df = df.join(
        mld_grouped,
        on=[df.index.get_level_values("time").month, "xlon", "ylat"],
        rsuffix="_clim",
    )
    ...
    df["T0"], df["T1"] = [
        np.cos(days_idx * 2 * np.pi / 365),
        np.sin(days_idx * 2 * np.pi / 365),
    ]
    df["A"], df["B"], df["C"] = [
        np.sin(lat_rad),
...
def create_inputs(
    ...
    lsmask_path,
    N_batch=12,
):  # N_batch should be number of months
    """
    ...
    """
    DS, DS_xco2 = import_member_data(
        ensemble_dir_head, ens, member, xco2_path, socat_path, dates
    )
    df = DS.to_dataframe()
    df = create_features(df, N_time, N_batch=N_batch)
    net_mask = np.tile(
        network_mask(topo_path, lsmask_path)
        .transpose("lon", "lat")
        .to_dataframe()["mask"]
        .to_numpy(),
        len(dates),
...
def evaluate_test(y, pred):
    ...
    scores = {
        ...
        "stdev": np.std(pred),
        "amp_ratio": (np.max(pred) - np.min(pred))
        / (np.max(y) - np.min(y)),  # added when doing temporal decomposition
        "stdev_ref": np.std(y),
...
def save_model(model, dates, model_output_dir, ens, member):
    ...
    try:
        ...
        print(
            f"Model successfully uploaded to GCS: gs://{gcs_bucket_name}/{destination_blob_name}"
        )
